...
* `krillScriptMarco.showCharRefsOfSayCommands`: 查看角色的引用时，是否应该查找位于说话指令中的引用。可以为`"none"`或`"all"`。
* `krillScriptMarco.showDialogCommandsInOutline`: 是否应该在大纲中显示对话的全部内容。可以为`"none"`, `"limited"`或`"all"`。
* `krillScriptMarco.showAnonymousDeclaresInOutline`: 是否应该在大纲中显示匿名声明，如匿名对话和匿名线索。可以为`"all"`, `"warp"`或`"none"`。
* `krillScriptMarco.showOmittedCharIds`: 是否应该通过镶嵌提示在说话指令中显示省略的角色 ID。可以为`"none"`或`"all"`。
* `krillScriptMarco.showDialogInfoAtDeclare`: 是否应该在对话声明处通过镶嵌提示显示统计到的对话信息，如所有说话者的角色 ID。可以为`"none"`或`"all"`。
...
由于语言服务功能不是实时更新的，有时会出现延迟甚至错位的情况。这个问题能带来的最严重的影响，就是重命名功能有可能会在错误的位置，破坏性地修改文档。
...
修复一个匿名符号名称错误的bug。（尽管这个bug几乎不影响使用）

允许在大纲中隐藏匿名符号。

### 0.0.11

调整魔法标识符的规则，修复代码段中的一个格式错误。

### 0.0.12

悬停在`dialog`等关键字上也可以显示对应声明的悬停信息，这对匿名声明非常有用。

省略说话者时，通过镶嵌提示显示说话者。

把大纲中询问对的冒号修正为箭头，如`詹姆斯：对话1`变为`詹姆斯 -> 对话1`。

### 0.0.13

修复说话者重复时不报错的bug。（其实是我忘检查返回类型了）

修复编译错误信息只有行号但没有文件名的bug。（其实是我忘写了）

### 0.0.14

通过镶嵌提示在对话声明处统计并显示该对话中的所有发言人。

### 0.0.15

通过镶嵌提示统计发言人时，对人名进行排序。

允许使用中文单引号来书写字符串。

### 0.0.16

更改匿名符号的生成规则，由递增数字改成随机数字。（因此，两次编译生成的文件几乎不可能兼容）

通过镶嵌提示统计发言人时，把原先的圆括号改成尖括号。
...
标识符不能是保留字，保留字有`char`, `clue`, `dialog`, `note`, `import`, `combine`和它们的中文版本`角色`, `线索`, `对话`, `笔记`, `导入`, `组合`（中文版本的关键字可以跟英文版本互换）。
...
`combine {<原因线索1> <原因线索2> <原因线索3>...} -> <结论线索>`用来声明一个组合。组合意味着把多个线索组合起来推理得出一个新线索。当玩家集齐了花括号中的所有原因线索时，自动获得结论线索。目前这个语法不受支持，以后应该永远也不会实装。
...
注意：尽管标识符是大小写敏感的，然而在最终的运行环境(Scratch)中，两个仅有大小写不同的标识符会相互混淆。但由于是我自用的，我稍微留心点就行了，不打算修了。

另外，匿名标识符的数量不能太多，差不多几千个可能就不太稳了，因为匿名标识符的名称只有4万多个。但还是那句话，反正我自用的。。。

用户不应该创建以`_ksm_`开头的标识符，如`_ksm_foo`。以`_ksm_`开头的标识符可能会被编译器占用，并且产生特殊或未定义的行为。
