...
标识符不能是保留字，保留字有`char`, `clue`, `dialog`, `note`, `import`, `combine`和它们的中文版本`角色`, `线索`, `对话`, `笔记`, `导入`, `组合`（中文版本的关键字可以跟英文版本互换）。并且用户不应该创建以`_ksm_`开头的标识符，如`_ksm_foo`。以`_ksm_`开头的标识符可能会被编译器占用，并且产生特殊或未定义的行为。
